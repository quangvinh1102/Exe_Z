...
public class Ranked {
    ...
    public static void initTopCaoThu() {
        try {
            ...
            Connection conn = DbManager.getInstance().getConnection(DbManager.SERVER);
            PreparedStatement stmt = conn.prepareStatement(
                    "SELECT `name`, CAST(JSON_EXTRACT(data, \"$.exp\") AS SIGNED) AS `exp`, CAST(JSON_EXTRACT(data, \"$.levelUpTime\") AS SIGNED) AS `levelUpTime` FROM players where `server_id` = ? ORDER BY `exp` DESC, `levelUpTime` ASC LIMIT 10;");
            stmt.setInt(1, Config.getInstance().getServerID());
            ResultSet res = stmt.executeQuery();
    ...
    public static void initTopClass(byte clazz) {
        try {
            ...
            Connection conn = DbManager.getInstance().getConnection(DbManager.SERVER);
            PreparedStatement stmt = conn.prepareStatement(
                    "SELECT `name`, CAST(JSON_EXTRACT(data, \"$.exp\") AS SIGNED) AS `exp`, CAST(JSON_EXTRACT(data, \"$.levelUpTime\") AS SIGNED) AS `levelUpTime` FROM players where `server_id` = ? and `class` = ? ORDER BY `exp` DESC, `levelUpTime` ASC LIMIT 10;");
            stmt.setInt(1, Config.getInstance().getServerID());
            stmt.setInt(2, clazz);
            ResultSet res = stmt.executeQuery();
            ArrayList<CaoThu> list = new ArrayList<>();
    ...
    public static void initTopHangDong() {
        try {
            ...
            Connection conn = DbManager.getInstance().getConnection(DbManager.SERVER);
            PreparedStatement stmt = conn.prepareStatement(
                    "SELECT `name`, `rewardPB` FROM `players` WHERE `rewardPB` > 0 AND `server_id` = ? AND CAST(JSON_EXTRACT(data, \"$.level\") AS SIGNED) BETWEEN 60 AND 69 ORDER BY `rewardPB` DESC LIMIT 10;");
            stmt.setInt(1, Config.getInstance().getServerID());
            ResultSet res = stmt.executeQuery();
            int i = 1;
            while (res.next()) {
...
//            stmt.setInt(1, Config.getInstance().getServerID());
    ...
    try {
        ...
        Connection conn = DbManager.getInstance().getConnection(DbManager.SERVER);
        PreparedStatement stmt = conn.prepareStatement(
           "SELECT `name`, CAST(JSON_EXTRACT(data, \"$.diemvxmm\") AS SIGNED) AS `diemvxmm` " +
           "FROM players WHERE `server_id` = ? AND CAST(JSON_EXTRACT(data, \"$.diemvxmm\") AS SIGNED) > 10 " +
